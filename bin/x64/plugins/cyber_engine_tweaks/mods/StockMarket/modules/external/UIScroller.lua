local Ref = require('modules/external/Ref')
local EventProxy = require('modules/external/EventProxy')

---@return inkCanvasWidget, inkSliderController
local function createWidget()
        local scrollArea = inkCanvas.new()
        scrollArea:SetName(StringToName('scroller'))
...
local function onWheelScroll(_, evt)
        ...
        if evt:IsAction("mouse_wheel") or evt:IsAction("right_stick_y_scroll") then
                ...
                local scrollContent = scrollArea:GetWidgetByPathName(StringToName('scrollContent'))
                if not scrollContent then return end

                local areaSize = scrollArea:GetSize()
                local contentSize = scrollContent:GetContentSize()
                if contentSize.Y <= areaSize.Y then return end

                local numVisibleChilds = 0
                local list = scrollContent:GetWidgetByPathName(StringToName('list'))
                if list then
                ...
                if numVisibleChilds <= 0 then numVisibleChilds = 1 end
...
function UIScroller.Create()
        local root = createWidget()
...
function UIScroller:GetPosition()
        return getScrollPosition(self.root)
end
